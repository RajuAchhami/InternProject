    ...
    if (newFilterText === "SHOW ALL") {
      ...
      setData(
        Courses.filter((course) =>
          course.category.toUpperCase() === newFilterText
        )
      );
    }
}
    ...
    return (
        ...
        <div className="flex flex-col items-center ">
            ...
            {data.map((item, index) => {
                return (
                <div
                    className="bg-[#f1f0fe] rounded-3xl overflow-hidden h-max  cursor-pointer"
                    key={index}
                >
                    ...
                    <div className="absolute flex items-center justify-center bg-[#7a6ad8] text-white text-lg font-bold rounded-full w-24 h-24 md:w-24 md:h-24 lg:w-32 lg:h-32 -top-5 -right-5 md:-top-4 md:-right-4 lg:-top-6 lg:-right-7     shadow-lg">
                        <p className="font-bold text-lg text-white">$ </p>
                        <span className="text-lg md:text-xl lg:text-2xl font-bold text-white mt-5 mr-6 ml-1">
                        {item.price}
                        </span>
                    </div>
                    </div>
                    ...
                    <p className="text-2xl lg:text-[20px] xl:text-3xl text-[#1e1e1e] font-semibold">
                        {item.title}
                    </p>
                    </div>
                </div>
